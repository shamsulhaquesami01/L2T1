...
// Hash 1: Polynomial Rolling Hash
...
public:
    HashTableBase(int size = 13) : tableSize(size), initialSize(size), 
                                   ...
                                   deletionsSinceCompact(0), totalCollisions(0), totalProbes(0) {}
    
    virtual ~HashTableBase() {}
    virtual void insert(const K& key, V value) = 0;
    virtual int search(const K& key) = 0; 
...
public:
    ...
    int search(const K& key) override {
        unsigned long long h = hashFunc(key);
        int idx = h % this->tableSize;
        int hits = 0;
    ...
    void rehash(int newSize) override {
        ...
        for (const auto& entry : oldTable) {
            if (entry.info == ACTIVE) {
                ...
                unsigned long long aux = auxHashFunc(entry.key);
                ...
                while (true) {
                     ...
                     if (table[idx].info != ACTIVE) break;
                     i++;
                }
...
int main() {
    ...
    auto runTest = [&](ResolutionMethod method, string methodName) {
        ...
        
        {
            ...
            for(const string& q : searchQueries) totalHits += table->search(q);
